...
class Investor {
  ...
  async setProportion(totalDesired) {
    ...
    console.log('proportion', this.proportion, this.getMax(), totalDesired); //save to db

    await (0, _lodash.delay)(() => {
  ...
  getRealInvestment(maxAllocation) {
    const money = maxAllocation * this.proportion; // return Math.ceil(money * 100) / 100; //round to cent
...
class Calculator {
  ...
  async getAllocations(investors, maxAllocation) {
    //build the investors
    const Investors = await this.buildInvestorList(investors); //what's the amount everyone wants together?

    const TOTAL_DESIRED = this.getTotalDesired(Investors); //calc each proportion
